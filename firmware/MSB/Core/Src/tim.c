...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if (htim == &htim16)//高优先级时基，控制所在时基 1ms
    {
        ...
        if (!(time_counter_for_tim16 % 5))//5ms时基
        {
            yuntai_control_driver();//云台控制;
        }
    ...
    if (htim == &htim17)//低优先级时基 100ms
    {
        if (!(time_counter_for_tim17 % 1))//100ms时基
        {
            ...
            //draw_motor
            if (yuntai_flags.motor_param_flag == 1 && yuntai_flags_control_enable_debug_mode) {
                draw_motor(2, draw_motor_last_state);
                draw_motor_last_state = 2;
            }
            else if (yuntai_flags.stable_control_flag == 1 && yuntai_flags_control_enable_debug_mode) {
                draw_motor(0, draw_motor_last_state);
                draw_motor_last_state = 0;
            }
            else if (yuntai_flags.pid_control_flag == 1 && yuntai_flags_control_enable_debug_mode) {
