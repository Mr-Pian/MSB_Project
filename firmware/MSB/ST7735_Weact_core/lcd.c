...
{
#ifdef TFT96
	ST7735Ctx.Orientation = ST7735_ORIENTATION_LANDSCAPE; //这里设置旋转
...
void LCD_SetBrightness(void)
{
	...
	HAL_GPIO_WritePin(LCD_BLK_GPIO_Port, LCD_BLK_Pin, GPIO_PIN_RESET); //打开背光
}
...
void LCD_Printf(uint16_t x, uint16_t y, uint8_t size, char* format, ...)
{
	...
	va_end(arg); //结束变量arg
	LCD_ShowString(x, y, 160 - 1, size, size, (uint8_t*)String);
}
