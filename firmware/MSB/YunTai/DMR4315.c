...
void motor_set_related_position(uint8_t id, int16_t pos) {
    ...
    data[7] = (uint8_t) crc;
    data[8] = (uint8_t) (crc >> 8);
    //HAL_UART_Transmit(&huart8, data, 9, 0xff);
    HAL_UART_Transmit_DMA(DMR_UART_HANDLER, data, 9);
    //	delay_ms(2);
    //	HAL_UART_Receive(&huart2,RxData,15,0xff);
...
void yuntai_motor_rec(uint8_t cnt, uint8_t *_uart_rx_buff) {
    /*解析协议*/
    uint8_t i = 0;
    while (i < cnt && (!(_uart_rx_buff[i] == 0x3c && _uart_rx_buff[i + 3] == ins && _uart_rx_buff[i + 1] == 0x00))) i++;
    //根据帧头寻找定位数据包
    ...
    if (_uart_rx_buff[i + length - 2] == (uint8_t) crc && _uart_rx_buff[i + length - 1] == (uint8_t) (crc >> 8)) {
        // 接收端CRC校验
        if (!(ins == 0x0c || ins == 0x0d || ins == 0x0e)) {
            //电机正常工作返回信息为编码器角度值和角速度值
            yuntai_motor[_uart_rx_buff[i + 2] - 1].val1 = (uint16_t) (
                _uart_rx_buff[i + 5] + (_uart_rx_buff[i + 6] << 8));//单圈绝对值角度  uint16_t
            yuntai_motor[_uart_rx_buff[i + 2] - 1].val2 = (int32_t) (
                _uart_rx_buff[i + 7] + (_uart_rx_buff[i + 8] << 8) + (_uart_rx_buff[i + 9] << 16) + (
                    _uart_rx_buff[i + 10] << 24));//多圈绝对值角度 int32_t
            yuntai_motor[_uart_rx_buff[i + 2] - 1].val3 = (int16_t) (
                _uart_rx_buff[i + 11] + (_uart_rx_buff[i + 12] << 8));//机械速度  int16_t

            yuntai_motor[_uart_rx_buff[i + 2] - 1].angle_s =
                (float) yuntai_motor[_uart_rx_buff[i + 2] - 1].val1 * (360.0f / 16384.0f);//单圈绝对值角度真实值 float
            yuntai_motor[_uart_rx_buff[i + 2] - 1].angle =
                (float) yuntai_motor[_uart_rx_buff[i + 2] - 1].val2 * (360.0f / 16384.0f);//多圈绝对值角度真实值 float
            yuntai_motor[_uart_rx_buff[i + 2] - 1].rpm = (float) yuntai_motor[_uart_rx_buff[i + 2] - 1].val2 * 0.1f;
            //机械速度rpm真实值 float
        } else {
            //配置指令  电机返回信息为配置参数
